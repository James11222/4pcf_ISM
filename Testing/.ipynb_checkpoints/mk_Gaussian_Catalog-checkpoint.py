...
class GenerateGRF(object):
    ...
    def __init__(self, Nmesh, Pk_in=None, los=None, which_rsd=None, do_inv_phase=False, bias=2.2,
                 mean_weight=1., cname=None, cname_GRF=None, cname_dfield=None, verbose=False):
        ...
        self.bias = bias #change this for scaling test
        self.mean_weight = mean_weight
        self.verbose = verbose

        if cname is not None:
            self.cname = cname # dir and name to save the catalog
        if cname_dfield is not None:
            self.cname_dfield = cname_dfield # dir and name to save the catalog
        if cname_GRF is not None:
            self.cname_GRF = cname_GRF # dir and name to save the catalog
    ...
    def save_catalog_ascii(self):
        """
        This function saves the data of a GRF. If cname_dfield is not None, it
        will save the density field in a .npy file. If cname_GRF is not None it 
        will save the GRF in the form of a catalog. This catalog is stored in a
        .txt file where the first 3 columns are position, the 4th column is 
        ...
        if hasattr(self, 'cname_dfield'):
            numpy.save(self.cname + self.cname_dfield + '.npy', self.deltar)
        if hasattr(self, 'cname_GRF'):
            numpy.savetxt(self.cname + self.cname_GRF + '.txt', cat_ascii)
